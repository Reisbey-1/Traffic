...
rar a -r %name%_%_my_datetime%.rar   -x*.obj -x*.ncb -x*.zip  -x*.pch -x*.pdb -x*.idb -x*.tlh -x*.tli -x*.aps -x*.opt -x*.lib -x*.bsc -x*.sbr -x*.ilk -x*.map -x*.cdf -x*.dbf -x*.dat -x*.bak -x*CVS/*.*  -x*\Debug -x*\Release -x*.vs -x*\ipch
